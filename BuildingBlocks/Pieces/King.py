class King:
    ...
    def possible_moves(self, board):
        ...
        if self.x != 0:
            if board[self.x - 1][self.y].piece is None:
        ...
        if (self.x != 0 and self.y != 0):
            if board[self.x - 1][self.y - 1].piece is None:
    ...
    def possible_captures(self, board):
        ...
        if self.x != 0:
            if board[self.x - 1][self.y].piece:
                if board[self.x - 1][self.y].piece.color != self.color:
        ...
        if self.y != 0:
            if board[self.x][self.y - 1].piece:
                if board[self.x][self.y - 1].piece.color != self.color:
    ...
    def possible_castling(self, board):
        ...
        if self.color == "white":
            if self.has_not_moved:
                ...
                if board[0][0].piece:
                    if board[0][0].piece.name == "Rook" and board[0][0].piece.has_not_moved:
                        # no pieces in between
                        if not board[1][0].piece and not board[2][0].piece and not board[3][0].piece:
                            possible_castling.append([(2, 0), (4, 0), (3, 0), (0, 0)])  # king new, king old, rook new, rook old
                # short castles
                # king and rook have not moved
                if board[7][0].piece:
                    if board[7][0].piece.name == "Rook" and board[7][0].piece.has_not_moved:
                        # no pieces in between
                        if not board[5][0].piece and not board[6][0].piece:
                            possible_castling.append([(6, 0), (4, 0), (5, 0), (7, 0)])  # king new, king old, rook new, rook old
        ...
        if self.color == "black":
            if self.has_not_moved:
                ...
                # king and rook have not moved
                if board[0][7].piece:
                    if board[0][7].piece.name == "Rook" and board[0][7].piece.has_not_moved:
                        # no pieces in between
                        if not board[1][7].piece and not board[2][7].piece and not board[3][7].piece:
                            possible_castling.append([(2, 7), (4, 7), (3, 7), (0, 7)])  # king new, king old, rook new, rook old
                # short castles
                # king and rook have not moved
                if board[7][7].piece:
                    if board[7][7].piece.name == "Rook" and board[7][7].piece.has_not_moved:
                        # no pieces in between
                        if not board[5][7].piece and not board[6][7].piece:
                            possible_castling.append([(6, 7), (4, 7), (5, 7), (7, 7)])  # king new, king old, rook new, rook old
